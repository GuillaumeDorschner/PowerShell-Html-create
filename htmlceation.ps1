...
<h3>Report of the " + (Get-Date -Format "dd") + " " + ((Get-Culture).DateTimeFormat.GetMonthName(8)) + " " + (Get-Date -Format "yyyy") + "</h3>
    <div class=`"rapport`">
        <ul>
            $(foreach($folder in $folders){
                "<li>" + (Get-ChildItem $folder -Include ($(foreach($extension in $extensions){"*" + $extension})) -recurse  -exclude $exclusions).Count + "<b> files </b>in the directory <b>" + (Split-Path $folder -Leaf) + "</b></li>"
            })
        </ul>
    </div>
</div>
